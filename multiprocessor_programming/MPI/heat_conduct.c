...
int main(int argc, char ** argv)
{
	...
	//..........................................//
	// сначала простой способ передачи сообщений
	...
	for ( i = 0; i < betw_iter; i++) {
		...
		// первый поток сначала пересылает данные, остальные сначала принимают, а потом пересылают соседу снизу
		if (start == 0) {
			if (my_rank + 1 != proc_number)
				MPI_Send(temp[str_num-1], size, MPI_FLOAT, 1, 1, MPI_COMM_WORLD);
		} else {
		...
		// теперь наоборот сначала последний пересылает, а остальные получают и пересылают соседу сверху
		if (end == size - 1) {
			if (my_rank + 1 != proc_number)
                                MPI_Send(temp[0], size, MPI_FLOAT, my_rank, my_rank, MPI_COMM_WORLD);
                } else {
                        MPI_Recv(buf, size, MPI_FLOAT, my_rank + 1, my_rank, MPI_COMM_WORLD, &status);
                }
                if (start != 0) {
	...
	for (str = 0; str < str_num; ++str) {
		//printf("rank %d ", my_rank);
                //for (col = 0; col < size; ++col)
                        printf(" %lf\t%lf", temp[str][0], str*h-h/2);
                printf("\n");
        }
	/*printf("\nbetw_temp\n");
	for (str = 0; str < str_num; ++str) {
